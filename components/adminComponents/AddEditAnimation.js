		...
		<aside className={open ? styles.mainContainer : styles.mainContainerClose}>
			...
			<form>
				...
				<div className={styles.opCategories}>
					<input
						type="text"
						placeholder="Categoría Optativa"
						maxLength="25"
						className={styles.input}
						value={tag}
					...
					<datalist id="categoryList">
						{autocomplete
							? autocomplete.map((tag, i) => <option value={tag} key={i} />)
							: null}
					</datalist>
					<button onClick={(e) => addTag(e)} className={styles.addTagBtn}>
						Agregar
					</button>
				</div>
				...
				{Object.keys(edit).length > 0 ? (
					<div className={styles.editDropContainer}>
						...
						<button className={styles.dropBtn} onClick={(e) => submitDelete(e)}>
							<DropIcon classN={styles.dropBtnIcon} />
						</button>
					</div>
				) : (
					<button
