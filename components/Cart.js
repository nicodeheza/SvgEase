...
export default function Cart({
	...
	function mpPay() {
		const mp = new MercadoPago("TEST-b6e0406f-2696-481b-b367-1ac83e93b342", {
			locale: "es-AR"
		});
	...
	return (
		<aside
			className={
				open && store
					? styles.storeCartMainContainer
					: !open && store
					? styles.storeCartMainContainerClose
					: open && !store
					? styles.cartMainContainer
					: styles.cartMainContainerClose
			}
		>
				...
				<tbody>
					{cartProducts.map((product, i) => (
						<tr
							key={i}
							className={
								userProducts?.includes(product._id)
									? styles.productRepeat
									: styles.productsTr
							}
						>
							...
							<td className={styles.priceContainer}>
								<h5>{`${
									currency === "ars"
										? priceInPesos(product.price, usaToArs)
										: product.price
								}$ ${currency.toUpperCase()}`}</h5>
							</td>
							<td>
			...
			{!auth ? (
				<div className={styles.btnContainer}>
					<button onClick={() => setFloatWin("logIn")}>
						Iniciar Sesión{" "}
						<span>
							<LoginIcon classN={styles.iconBtn} />
						</span>
					</button>
					<button onClick={() => setFloatWin("singUp")}>
						Crear Cuenta{" "}
						<span>
							<SingupIcon classN={styles.iconBtn} />
						</span>
					</button>
				</div>
			) : total <= 0 ? (
				<div className={styles.btnContainer}>
					<p style={{fontFamily: "var(--text-font)", color: "red"}}>Carro vacío</p>
				</div>
			) : repeat ? (
				<div className={styles.btnContainer}>
					<p style={{fontFamily: "var(--text-font)", color: "red", width: "80%"}}>
						Ya tienes uno o mas productos en tu cuenta, quita los del carro para poder
						pagar.
					</p>
				</div>
			) : (
					...
					{currency === "ars" ? (
						<>
							<Image
								...
								height={60}
							/>
							<button onClick={() => mpPay()}>Pagar</button>
						</>
					) : (
						<>
							<PayPal
