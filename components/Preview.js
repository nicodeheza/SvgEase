	...
	function addToCart() {
		...
		if (!activeBtn) {
			...
			localStorage.setItem(
				"cart",
				JSON.stringify(cart.filter((ele) => ele._id !== product._id))
			);
			setActiveBtn(false);
		}
		setUpdateCart(true);
		...
		<div className={styles.mainContainer}>
			<div className={styles.container}>
				...
				<div style={{backgroundColor: bkColor}} className={styles.animationContainer}>
					<div
						id={`productPreviewAnimation${product._id}`}
						className={styles.animation}
					/>
				</div>
				<div className={styles.playContainer}>
					<img
						src={!play ? "/svgs/play.svg" : "/svgs/pause.svg"}
						alt={play ? "play" : "pause"}
				...
				<div className={styles.priceBtn}>
					<p>{`${
						currency === "ars" ? priceInPesos(product.price, usaToArs) : product.price
					}$ ${currency.toUpperCase()}`}</p>
					<button
						onClick={() => (store ? addToCart() : addEdit())}
